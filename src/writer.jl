...
```
"""
function Writer(output::IO, chromlist::Union{AbstractVector,AbstractDict}; binsize::Integer=64, datatype::Symbol=:bedgraph)
...
function Base.write(writer::Writer, interval::GenomicFeatures.AbstractGenomicInterval{T}) where T<:Real
...
function finish_section!(writer::Writer)
    ...
    size = BBI.compress!(state.compressed, take!(state.buffer))
